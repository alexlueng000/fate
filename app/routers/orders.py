...
def create_order(req: CreateOrderRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def prepay(req: PrepayRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
