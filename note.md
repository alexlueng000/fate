...
=== fateinsight.site Nginx SSE 流式输出配置 ===

问题：fateinsight.site 所有请求先经过 Next.js（:3000），Next.js 的 rewrite 代理会缓冲 SSE 流，
导致 /api/chat、/api/chat/start、/api/chat/simplify 等接口内容堆积后一次性返回，无法流式输出。

解决方案：在 /etc/nginx/sites-available/fateinsight.site.conf 中为 /api/chat 路由添加 location 块，
直接绕过 Next.js，代理到 FastAPI（:8000）：

server {
    listen 443 ssl http2;
    listen [::]:443 ssl http2;
    server_name fateinsight.site www.fateinsight.site;

    ssl_certificate     /etc/letsencrypt/live/fateinsight.site/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/fateinsight.site/privkey.pem;

    # SSE 流式接口 —— 直接走 FastAPI，绕过 Next.js
    # 覆盖：/api/chat、/api/chat/start、/api/chat/simplify
    location ~* ^/api/chat {
...
注意：location ~* ^/api/chat 必须放在 location / 前面。
